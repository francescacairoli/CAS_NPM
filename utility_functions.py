...
def label_correct_incorrect_pred(predicted_lkh, real_class):


	n_points, n_classes, n_outputs = predicted_lkh.shape # 1 = correct label; -1 wrong label

	error_indexes = np.ones((n_points,n_outputs))
	for i in range(n_points):
		for j in range(n_outputs):
			#print(real_class[i,j], " ---- ", np.argmax(predicted_lkh[i,:,j]))
			if real_class[i,j] != np.argmax(predicted_lkh[i,:,j]):
				error_indexes[i,j] = -1
...
def plot_test_results_w_coverage(dataset, estim_states, width, path, ds_type = ""):

	plots_path = path+"/SeqSE_Plots"+ds_type
	os.makedirs(plots_path, exist_ok=True)
